...
NSString * Dorosh_Ian_UtilityApp$ModuleRequireFormat = @"(function(exports){"
...
@implementation Dorosh_Ian_UtilityAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_dorosh_ian_utilityapp);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Dorosh_Ian_UtilityApp global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Dorosh_Ian_UtilityApp" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_dorosh_ian_utilityapp = [[Dorosh_Ian_UtilityAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_dorosh_ian_utilityapp];
	
	NSString *_dorosh_ian_utilityappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _dorosh_ian_utilityappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_dorosh_ian_utilityapp valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Dorosh_Ian_UtilityApp$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.dorosh_ian_utilityapp.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.dorosh_ian_utilityapp.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
