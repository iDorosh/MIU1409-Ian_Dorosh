...
NSString * Dorosh_Ian_InfoApp$ModuleRequireFormat = @"(function(exports){"
...
@implementation Dorosh_Ian_InfoAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_dorosh_ian_infoapp);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Dorosh_Ian_InfoApp global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Dorosh_Ian_InfoApp" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_dorosh_ian_infoapp = [[Dorosh_Ian_InfoAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_dorosh_ian_infoapp];
	
	NSString *_dorosh_ian_infoappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _dorosh_ian_infoappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_dorosh_ian_infoapp valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Dorosh_Ian_InfoApp$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.dorosh_ian_infoapp.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.dorosh_ian_infoapp.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
